...
10. **Probe for secondary GraphQL endpoints with introspection enabled** — Large sites often expose MULTIPLE GraphQL endpoints: a main shared endpoint (e.g., `/graph/` or `/api/graphql`) where introspection is disabled and many ops require persisted-query hashes, PLUS one or more product-specific endpoints (e.g., `/fly/graph/query`, `/checkout/graph/query`) that may have introspection enabled. After finding the primary endpoint, grep JS bundles for alternate paths matching `/graph[a-z\/]*` or `/query` and test each with `{ __type(name: "Query") { fields { name } } }`. When introspection IS available on any endpoint, that's the fastest path to a complete API surface map — enumerate Query/Mutation fields, then recursively introspect each input type to discover the full schema without reverse-engineering from network captures.

**HARD RULE: The AI must exhaust EVERY discovery technique (all 10 above) before concluding that an endpoint or operation cannot be done.** Do not give up after trying 1-2 approaches. Do not say "this can't be done" after a few failed fetch calls. The correct mindset is: "the web app does this operation in the browser — therefore there IS a way to do it programmatically, and I MUST find it." When the obvious approach fails, escalate: try network capture during UI interaction, search JS bundles, try different headers (`X-Requested-With`, `Accept`, `Turbo-Frame`), try form submission, try GraphQL mutations. Only after genuinely exhausting every technique AND confirming with the user should you report that an operation is not feasible. Being lazy or giving up early is unacceptable — it results in plugins that lack critical functionality the user expects.
