...
/**
 * Recover from a stale Graph token on SharePoint/OneDrive-hosted presentations.
 *
...
export const reauthenticate = defineTool({
  ...
  description:
    ...
    'Use this when another PowerPoint tool returns an authentication error pointing at this tool. ' +
  ...
  handle: async () => {
    ...
    // `powerpoint.cloud.microsoft` app has no encrypted-cache problem, so
    // there is nothing to clear and no reload to perform.
    if (!isSharePointPresentation()) {
      return {
